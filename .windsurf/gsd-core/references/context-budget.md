...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GSD itself can save through `model_profile` tuning. This is a Windsurf harness concern, not a GSD concern: GSD does **not** manage MCP enablement. The toggle lives in `.windsurf/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
The keys live in `.windsurf/settings.json` (project) or `/Users/smintz/git/platform-engineering/.windsurf/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
