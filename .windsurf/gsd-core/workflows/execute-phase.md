...
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/agent-contracts.md
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/context-budget.md
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/gates.md
</required_reading>

<available_agent_types>
These are the valid GSD subagent types registered in .windsurf/agents/ (or equivalent for your runtime).
...
**Model resolution:** If `executor_model` is `"inherit"`, omit the `model=` parameter from all `Agent()` calls — do NOT pass `model="inherit"` to Agent. Omitting the `model=` parameter causes Windsurf to inherit the orchestrator model automatically. Only set `model=` when `executor_model` is an explicit model name (e.g., `"claude-sonnet-5"`, `"claude-opus-4-8"`).

@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-phase-response-language.md

Read runtime/worktree config and fail closed before any executor dispatch:

```bash
RUNTIME=$(gsd_run query config-get runtime --default windsurf --raw 2>/dev/null || echo "windsurf")
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/planner-antipatterns.md
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/workflows/execute-plan.md` **inline**
...
`.windsurf/worktrees/`, so a path-substring refusal would break legitimate runs. Do NOT
...
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-phase-wave-guard.md

@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-phase-context-guard.md
   ...
   Agent(
    ...
    description="Execute plan {plan_number} of phase {phase_number}",
     ...
     prompt="
       ...
       - `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/checkpoints.md`
       ${TDD_APPLICABLE ? '- `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/tdd.md`' : ''}  # #3990/#4265: type: tdd, tdd="true", or workflow.tdd_mode
       - `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/worktree-path-safety.md`
       ${CONTEXT_WINDOW < 200000 ? '' : '- `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/executor-examples.md`'}
       </execution_context>
...
4. **Wait for all agents in wave to complete.**
   ...
   **This fallback applies to all runtimes.** Windsurf's Agent() backgrounds by
...
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-phase-quota-recovery.md

@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-phase-between-wave-reset.md
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-mvp-tdd.md`): when `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit; #4011 — no MVP condition), the end-of-phase TDD review escalates from advisory to **blocking under TDD** — refuse to mark the phase complete and present:
...
**If gaps_found:**
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/references/execute-phase-requirement-revert.md
```
...
@/Users/smintz/git/platform-engineering/.windsurf/gsd-core/workflows/transition.md
</step>
