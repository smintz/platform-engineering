...
if [ -z "$PATCHES_DIR" ] && [ -n "$WINDSURF_CONFIG_DIR" ]; then
  candidate="$(expand_home "$WINDSURF_CONFIG_DIR")/gsd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsd-local-patches" ]; then
    ...
    PATCHES_DIR="/Users/smintz/git/platform-engineering/.windsurf/gsd-local-patches"
  fi
fi
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.windsurf/` → `/Users/smintz/git/platform-engineering/.windsurf/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
