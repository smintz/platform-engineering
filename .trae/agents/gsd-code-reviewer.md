...
**Project instructions:** Read `.trae/rules/rules.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**5. Load project context:** Read `.trae/rules/rules.md` and check for `.trae/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO consider project conventions** from .trae/rules/rules.md when evaluating code quality. What's a violation in one project may be standard in another.
