...
**Project instructions:** Read `.trae/rules/rules.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
if [ "$USE_WORKTREES" = "false" ]; then
  ...
  # #2647: create the worktree INSIDE the repo under the same `.trae/worktrees/`
  ...
  # `.gitignore` rule for `.trae/` (`.gitignore:12`). Uniqueness across
  ...
  wt="$main_repo/.trae/worktrees/rf-${padded_phase}-$$-$(date +%s)"
  mkdir -p "$wt"
...
4. Create a unique worktree path **inside the repo**: `main_repo="$(git worktree list --porcelain | awk '/^worktree / { sub(/^worktree /, ""); print; exit }')"` then `wt="$main_repo/.trae/worktrees/rf-${padded_phase}-$$-$(date +%s)"` + `mkdir -p "$wt"`. The path lives under the same `.trae/worktrees/` dir the harness-managed executor worktrees use (already gitignored via `.trae/`, already in the session's permission scope), and the `$$`-PID + epoch suffix ensures concurrent runs for the same phase do not collide (#2647 — an absolute `/tmp` path landed outside the project tree and prompted on every read).
...
Read `.trae/rules/rules.md` and check for `.trae/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect .trae/rules/rules.md project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from .trae/rules/rules.md respected during fixes
