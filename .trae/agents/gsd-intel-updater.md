...
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "@opengsd/gsd-core" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .trae/gsd-core 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
counts by running Glob on the layout-resolved canonical locations above, not from memory or .trae/rules/rules.md.
