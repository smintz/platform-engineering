...
@/Users/smintz/git/platform-engineering/.trae/gsd-core/references/few-shot-examples/plan-checker.md
...
## Dimension 10: .trae/rules/rules.md Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from .trae/rules/rules.md?

**Process:**
1. Read `.trae/rules/rules.md` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .trae/rules/rules.md explicitly forbids
5. Flag plans that skip steps .trae/rules/rules.md explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .trae/rules/rules.md explicitly forbids
- Plan skips a required step (e.g., .trae/rules/rules.md says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .trae/rules/rules.md conventions
- Plan creates files in locations that violate .trae/rules/rules.md's architectural constraints
- Plan ignores security requirements documented in .trae/rules/rules.md

**Skip condition:** If no `.trae/rules/rules.md` exists in the working directory, output: "Dimension 10: SKIPPED (no .trae/rules/rules.md found)" and move on.
...
issue:
  ...
  severity: blocker
  required_property: "Plans use the toolchain .trae/rules/rules.md mandates"
  description: "Plan uses Jest for testing but .trae/rules/rules.md requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .trae/rules/rules.md"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  required_property: "Every `<verify>` runs the checks .trae/rules/rules.md requires"
  description: "Plan does not include lint step required by .trae/rules/rules.md"
  plan: "02"
...
@/Users/smintz/git/platform-engineering/.trae/gsd-core/references/plan-checker-examples.md
...
see contradicts a locked decision, a .trae/rules/rules.md convention, or an active capability constraint. If
...
- [ ] .trae/rules/rules.md compliance checked (plans respect project conventions)
