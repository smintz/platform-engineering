...
@/Users/smintz/git/platform-engineering/.trae/gsd-core/references/git-integration.md
</required_reading>
...
2. **MCP tools:** If .trae/rules/rules.md or project instructions reference MCP tools (e.g. jCodeMunch for code navigation), prefer them over Grep/Glob when available. Fall back to Grep/Glob if MCP tools are not accessible.
3. Per task:
...
exactly as specified in the canonical `/Users/smintz/git/platform-engineering/.trae/gsd-core/references/tdd.md` reference — the
...
bypass violates project .trae/rules/rules.md guidance. If a project explicitly opts out via
...
See /Users/smintz/git/platform-engineering/.trae/gsd-core/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@./.trae/gsd-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/Users/smintz/git/platform-engineering/.trae/gsd-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Users/smintz/git/platform-engineering/.trae/gsd-core/templates/summary.md`.
