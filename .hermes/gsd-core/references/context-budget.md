...
The keys live in `.hermes/settings.json` (project) or `/Users/smintz/git/platform-engineering/.hermes/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/smintz/git/platform-engineering/.hermes/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
