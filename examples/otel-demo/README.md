...
```bash
make compile      # src/otel_demo.mpconf -> outputs/otel_demo/<domain>/<service>/{infra,monitoring}/main.tf.json
...
Or on GKE, where the same inventory also provisions the cluster it deploys onto — a VPC, a
subnet, a router, a NAT and a zonal cluster, from the [GKE driver](../../drivers/cluster/gke):

```bash
gcloud auth application-default login
make apply DOMAIN=gke-asia-southeast3          # network, cluster, then every service
make credentials DOMAIN=gke-asia-southeast3    # a kubeconfig for kubectl alone
make monitoring DOMAIN=gke-asia-southeast3
make destroy DOMAIN=gke-asia-southeast3        # services first, then the cluster under them
```

Two domains, rendered from one inventory: `local` is whatever cluster kubectl points at,
and `gke-<region>` is a cluster of this stack's own. Set `PROJECT` in
`src/components/defaults.pinc` before applying that one; the state lives in a GCS bucket
named after it, which has to exist first.
...
src/components/defaults.pinc   the domains, the cluster each runs on, what every component
                               gets (dashboards), and where state lives
...
- **`src/terraform`:** the vendored provider protos, generated by `protoconf-terraform` from
  the kubernetes, grafana and google providers this stack uses, and pruned with its
  `optimize` to the messages the configs actually build.
