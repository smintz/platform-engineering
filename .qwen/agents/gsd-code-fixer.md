...
if [ "$USE_WORKTREES" = "false" ]; then
  ...
  # #2647: create the worktree INSIDE the repo under the same `.qwen/worktrees/`
  ...
  # `.gitignore` rule for `.qwen/` (`.gitignore:12`). Uniqueness across
  ...
  wt="$main_repo/.qwen/worktrees/rf-${padded_phase}-$$-$(date +%s)"
  mkdir -p "$wt"
...
4. Create a unique worktree path **inside the repo**: `main_repo="$(git worktree list --porcelain | awk '/^worktree / { sub(/^worktree /, ""); print; exit }')"` then `wt="$main_repo/.qwen/worktrees/rf-${padded_phase}-$$-$(date +%s)"` + `mkdir -p "$wt"`. The path lives under the same `.qwen/worktrees/` dir the harness-managed executor worktrees use (already gitignored via `.qwen/`, already in the session's permission scope), and the `$$`-PID + epoch suffix ensures concurrent runs for the same phase do not collide (#2647 — an absolute `/tmp` path landed outside the project tree and prompted on every read).
