...
The keys live in `.qwen/settings.json` (project) or `/Users/smintz/git/platform-engineering/.qwen/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/smintz/git/platform-engineering/.qwen/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
